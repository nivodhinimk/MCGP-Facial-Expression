...
for j = 1:nn
    ...
    % Handle non-uint8 formats (tif, etc.)
    if ~isa(img1, 'uint8')
        img1 = im2uint8(img1);
    end
...
% Fuse AFTER the loop, not inside it
featuress = [hogf, lbpf];
...
numDisplay = min(30, numImages);

perm = randperm(numImages, numDisplay);

dd = cell(1, numDisplay);
...
helpdlg('Sample images displayed');
